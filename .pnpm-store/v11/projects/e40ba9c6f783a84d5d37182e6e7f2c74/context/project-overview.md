...
```
/                        → Public landing page (available to every visitor)
/home                    → Authenticated map-veil launch surface
...
### 1. Public Landing and Authenticated Home

- `/` is the public landing page for both signed-out and signed-in visitors.
- After sign-up or login, users land on protected `/home`, a map-veil launch surface with a direct route to flight search.
- `/home` redirects signed-out visitors to `/login`.

### 2. Onboarding

- User signs up with email and password (social login deferred to a future milestone).
- On login or post-registration auto-login → redirect to `/home`.
...
```mermaid
flowchart TD
    A["Public Landing (/)"] --> B["Login / Sign Up"]
    B --> C["Authenticated Home (/home)"]
    C --> D["Search Flights"]
