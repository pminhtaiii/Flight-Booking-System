...
```text
GET / → public LandingPage for every visitor

GET /home → getServerSession(authOptions)
...
Successful login and registration route to `/home`; the authenticated launch CTA links to `/search`. The airport map is decorative and non-interactive; airport-data, tile, or WebGL failures leave the authenticated shell usable.
