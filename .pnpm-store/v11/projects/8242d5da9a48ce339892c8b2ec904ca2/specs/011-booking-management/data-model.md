...
2. **Read-Time Reactive Fail**: On list or detail API queries, if a `PROCESSING` booking was created more than 15 minutes ago (`createdAt <= NOW() - 15 minutes`), the backend service MUST reconcile the state before returning a status:
   ...
   - **External API Timeout & Fallback (Low Latency Guard)**: Any external Stripe or Duffel API calls executed during this read-time check MUST have a strict timeout of **3 seconds**. If the external calls time out or fail, the service MUST NOT block or fail the request with a 500 error. Instead, it must log the failure, bypass the update, and return the booking in its current `PROCESSING` state. The background cron job (which operates outside the user request path) will handle the reconciliation asynchronously on its next run.
