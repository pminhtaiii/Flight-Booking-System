...
const failureMessages: Record<BookingFailureReason, string> = {
  ...
  [BookingFailureReason.PRICE_CHANGED]: 'The price for this flight changed before we could confirm it.',
  ...
  [BookingFailureReason.SYSTEM_ERROR]: 'We could not complete this booking because of a system error.',
};
...
export function BookingFailureState({
  ...
  const routeQuery = firstSegment
    ...
    : '';
  const message = failureReason ? failureMessages[failureReason] : failureMessages[BookingFailureReason.SYSTEM_ERROR];

  return (
    <section aria-labelledby="booking-failed-title" className="card border border-text-cancelled">
      <h1 id="booking-failed-title" className="text-xl font-bold text-text-cancelled">We could not complete your booking</h1>
      ...
      <div className="mt-5">
        {failureReason === BookingFailureReason.OFFER_EXPIRED ? (
          <Link href={`/search${routeQuery}`} className="btn-primary">Search flights again</Link>
        ) : failureReason === BookingFailureReason.CAPTURE_FAILED ? (
          <a href="mailto:support@flightsystem.example" className="btn-secondary">Contact support</a>
        ) : (
          <Link href={offerId ? `/search/${offerId}` : '/search'} className="btn-primary">Review this flight</Link>
        )}
      </div>
    </section>
  );
