...
import { DisruptionStatus, MaterialDisruptionReason, DisruptionResolvedReason } from '@shared/disruption-types';
...
export const REASON_LABELS: Record<MaterialDisruptionReason, string> = {
  ...
  [MaterialDisruptionReason.DEPARTURE_MOVED_EARLIER]: 'Departure time moved earlier by more than 1 hour',
  [MaterialDisruptionReason.DEPARTURE_MOVED_LATER]: 'Departure time moved later by more than 2 hours',
  [MaterialDisruptionReason.FINAL_ARRIVAL_MOVED_EARLIER]: 'Arrival time moved earlier by more than 1 hour',
  [MaterialDisruptionReason.FINAL_ARRIVAL_MOVED_LATER]: 'Arrival time moved later by more than 2 hours',
  [MaterialDisruptionReason.OVERNIGHT_CONNECTION_INTRODUCED]: 'An overnight connection was introduced',
        ...
        <div className="space-y-2 flex-1">
          ...
          <div className="text-sm space-y-1">
            {status === DisruptionStatus.DETECTED && (
              <p>
                The airline has updated the schedule. Please review the changes below and take action.
              </p>
            )}
            {status === DisruptionStatus.ACKNOWLEDGED && (
              <p>
                You have acknowledged the changes. Please confirm whether you accept the updated itinerary.
              </p>
            )}
            {status === DisruptionStatus.RESOLVED && (
              <p>
                {resolvedReason ? RESOLVED_REASON_LABELS[resolvedReason] : 'This flight disruption has been resolved.'}
              </p>
            )}
          </div>
          ...
          {materialReasons && materialReasons.length > 0 && (
            <div className="mt-3 pt-3 border-t border-current/10">
              <h4 className="text-xs font-semibold uppercase tracking-wider opacity-90 mb-1.5">Reasons for change:</h4>
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm mt-1">
              Airline schedules are currently volatile. This itinerary may undergo further revisions.
            </p>
          </div>
        </div>
      )}
