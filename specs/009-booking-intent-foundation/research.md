...
- Phase 1 cron expiry runs as a single conditional update: `UPDATE booking_intents SET status = 'EXPIRED', updatedAt = now() WHERE status = 'PENDING' AND intentExpiresAt < now()`. It can only ever move a row _out of_ `PENDING`; it never matches rows already in `COMPLETED` or `EXPIRED`.
