...
**Branch**: `009-booking-intent-foundation` | **Date**: 2026-07-10 | **Spec**: [spec.md](file:///c:/Booking%20Systems/specs/009-booking-intent-foundation/spec.md)
**Input**: Grilling session decisions from [research](file:///c:/Booking%20Systems/research/booking-workflow-decisions.md)
...
file:///c:/Booking%20Systems/specs/009-booking-intent-foundation/data-model.md
)
 for full field list 
|
|
...
|
 ☐ 
|
 Injectable NestJS service, uses 
`ENCRYPTION_KEY`
 env var 
|
|
 Add shared types to 
`packages/shared/src/types/booking-intent.types.ts`
|
 ☐ 
|
`BookingIntentStatus`
...
`PassengerType`
 enums 
|
**Exit criteria**: Migration applied, both tables exist, Prisma client types generated, encryption service works with AES-256-GCM.
---
...
|
 ☐ 
|
 Passenger array, cross-field validation (infants ≤ adults, total ≤ 9) 
|
|
 Create 
`intent-response.dto.ts`
 for creation and retrieval responses 
|
 ☐ 
|
 Separate creation vs. detail shapes (passport excluded from creation response) 
|
|
 Create 
...
|
 ☐ 
|
 Core logic: validate → lookup FlightOffer → re-price via Duffel → encrypt PII → create intent + passengers in transaction 
|
|
 Implement TravelerProfile pre-fill logic 
|
 ☐ 
|
 If 
`useProfile: true`
 on primary adult, merge profile data into passenger fields before validation 
|
|
 Implement Duffel re-pricing (reuse 
`duffel.offers.get()`
 pattern) 
|
 ☐ 
|
 Same approach as 
`getFlightDetail()`
 — call Duffel, snapshot the response 
|
|
 Create 
...
|
 ☐ 
|
 JWT-guarded, ownership enforcement 
|
|
 Create 
`booking-intent.module.ts`
 and register in 
`app.module.ts`
|
 ☐ 
|
 Import PrismaModule, DuffelModule, AuditModule 
|
|
...
|
 ☐ 
|
 Structured metadata: intentId, userId, offerId, passengerCount, priceChanged 
|
**Exit criteria**: All 3 endpoints work. Intent creation validates passengers, re-prices via Duffel, encrypts PII, creates DB records in a transaction, and returns confirmed pricing. Retrieval decrypts passport data for the owning user. Pre-fill returns TravelerProfile data.
---
...
|
 Create 
`booking-intent.cron.ts`
 with 
`@nestjs/schedule`
|
 ☐ 
|
 Two cron methods: Phase 1 (soft expire) and Phase 2 (hard delete) 
|
|
 Phase 1 cron: 
`PENDING`
...
`BOOKING_INTENT_TTL_MINUTES`
 env var (default: 30) 
|
|
 Phase 2 cron: hard-delete 
`EXPIRED`
 rows when 
`updatedAt`
 + grace < now 
...
`BOOKING_INTENT_GRACE_HOURS`
 env var (default: 24) 
|
|
 Cascade delete verification: 
`BookingIntentPassenger`
 rows deleted with intent 
|
 ☐ 
|
 Prisma 
...
|
|
 Structured logging for each cron run 
|
 ☐ 
|
 Count of intents expired, count deleted, duration 
|
|
 Audit log entries for 
`booking_intent_expired`
 and 
`booking_intent_deleted`
|
 ☐ 
|
 Batch audit log with intent IDs 
|
|
 Register 
`ScheduleModule`
...
`@nestjs/schedule`
 may already be in 
`AppModule`
 from FlightOffer cron 
|
**Exit criteria**: Both cron phases run on schedule. PENDING intents expire after TTL. EXPIRED intents are hard-deleted after grace period. Cascading delete removes passenger rows. All operations logged and audited.
---
...
|
|
 E2E: Create intent with valid passengers (201) 
|
 ☐ 
|
 Verify response shape, DB records, encrypted passport data 
|
|
 E2E: Create intent with pre-fill from TravelerProfile 
|
 ☐ 
|
 Verify profile data merged into passenger 
|
|
 E2E: Validation — infants > adults (400) 
|
 ☐ 
|
...
|
|
 E2E: Validation — total passengers > 9 (400) 
|
 ☐ 
|
 Cross-field validation 
|
|
 E2E: Validation — passenger count mismatch (400) 
|
 ☐ 
|
 Array length ≠ declared breakdown 
|
|
 E2E: Retrieve own intent (200) 
|
 ☐ 
|
 Verify decrypted passport data in response 
|
|
 E2E: Retrieve other user's intent (403) 
|
 ☐ 
|
...
|
|
 E2E: Retrieve expired intent (410) 
|
 ☐ 
|
 Status = EXPIRED → 410 Gone 
|
|
 E2E: Pre-fill endpoint with profile (200) 
|
 ☐ 
|
 Verify profile data and missing fields 
|
|
 E2E: Pre-fill endpoint without profile (200) 
|
 ☐ 
|
`hasProfile: false`
|
|
 E2E: Cron Phase 1 — PENDING → EXPIRED 
|
 ☐ 
|
 Artificially age intent, trigger cron, verify status change 
|
|
 E2E: Cron Phase 2 — EXPIRED → deleted 
|
 ☐ 
|
 Artificially age expired intent, trigger cron, verify deletion + cascade 
|
|
 E2E: Duffel offer expired during re-pricing (410) 
|
 ☐ 
|
 Mock Duffel 404/410, verify error response 
|
|
...
|
 ☐ 
|
 No breakage from schema changes 
|
**Exit criteria**: All E2E tests pass. No regression on existing functionality. Encryption verified in DB. Cron lifecycle validated end-to-end.
---
...
|
 Yes 
|
 32-byte hex key for AES-256-GCM. Must be set in 
`.env`
|
|
`BOOKING_INTENT_TTL_MINUTES`
...
|
 No 
|
 Minutes before PENDING intent is marked EXPIRED 
|
|
`BOOKING_INTENT_GRACE_HOURS`
|
...
|
 No 
|
 Hours before EXPIRED intent is hard-deleted 
|
...
- Check `booking_intent_passengers` table → verify passport fields are encrypted (not plaintext)
