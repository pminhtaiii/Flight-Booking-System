...
- [x] T044 [US3] Implement mock/API/Agent/Web spawning, per-process safe logs, readiness, spec-reporter smoke→sanity sequencing, signal handling, and exact-child cleanup in `scripts/ci/run-smoke-sanity.mjs` (depends on T042)
- [x] T045 [US3] Implement an explicit local database reset mode that parses `DATABASE_URL` and refuses any database name other than `smoke_test` in `scripts/ci/run-smoke-sanity.mjs` (depends on T043)
