...
- **T090:** Direct browser requests now create separate `chat_<32 lowercase hex>` trace and correlation headers. Agent SSE sanitizes both values before constructing the NestJS client, and the client forwards only the sanitized values on service gateway calls; proxy fallback remains unchanged.
- **GREEN evidence:** The user-provided focused Playwright run passed. From `apps/agent`, `uv run pytest -q tests/test_direct_stream.py tests/test_nestjs_client.py` passed `34` tests with 3 known warnings; Ruff passed. From `apps/web`, targeted TypeScript checking of `lib/chatStream.ts` and `lib/featureFlags.ts` passed, and ESLint passed for `lib/chatStream.ts`.
