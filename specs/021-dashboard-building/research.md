...
- The prototype's `/prototype/chat` action links have no production counterpart. The global layout already mounts `ChatWidget`, so the assistant remains accessible there; the quick-action grid uses only `/search`, `/bookings?tab=upcoming`, `/bookings?tab=past`, and `/profile`. A generic disruption-center tile is deferred until a real production route and contract exist.
