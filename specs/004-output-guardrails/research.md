...
**Decision**: Add a `validate_output_chunk` method to the `NemoGuardrailService` (or a new `OutputGuardrailService` class) that calls the same Mimo endpoint with an output-specific system prompt:
