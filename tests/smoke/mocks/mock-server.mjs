...
function isValidOfferRequest(payload) {
  ...
  return data.slices.every((slice) => (
    slice
    ...
    passenger && typeof passenger.type === 'string' && passenger.type.length > 0
  ));
}
...
function buildOfferRequest(payload) {
  ...
  const mappedSlices = slices.map((slice, index) => ({
    ...
    duration: 'PT2H10M',
    origin: { id: slice.origin, name: `${slice.origin} Airport`, iata_code: slice.origin, type: 'airport' },
    destination: { id: slice.destination, name: `${slice.destination} Airport`, iata_code: slice.destination, type: 'airport' },
    segments: [{
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  runCli().catch((error) => {
    process.stderr.write(`Mock server failed to start: ${error instanceof Error ? error.message : 'unknown error'}\n`);
    process.exitCode = 1;
  });
}
