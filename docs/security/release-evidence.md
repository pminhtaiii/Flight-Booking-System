...
| **5** | **Local DAST & Adversarial Replay** | `node scripts/security/run-local-dast.mjs --profile full` | `0` | `15,100 ms` (~15s) | **45 endpoints evaluated**, **700 holdout cases**, **25 invariants verified** across detector and quota profiles via live replay engine (`scripts/security/run_dast_replay.py`). Reports generated to `artifacts/security/`. 0 Critical, 0 High findings. | **PASS** |
...
| `artifacts/security/dast.json` | JSON (Sanitized) | 268 bytes | `3E63E7A84B034957E33C55D75A9B375002D10B91ED1D8227B70CBA287A44CBF9` | Dynamic application security testing report across 45 cataloged endpoints. 0 Critical, 0 High findings, exit code 0. |
| `artifacts/security/detector-corpus.json` | JSON | 676 bytes | `FEFA9C48683C3887C6F3AD1C34552EB8BF872DAD8721D4B1C58513681F68028C` | Holdout detector corpus report across 700 frozen cases (input: 350, tool: 175, output: 175). Aggregate TPR = 100.00%, FPR = 0.00%, 0 reachability breaches. |
| `artifacts/security/invariant-corpus.json` | JSON | 3,475 bytes | `8CFE5D04DC9BB476B91DFF3C83B4894F4BC7F93879A14D9081D1E58E7DBFB39B` | Security invariant corpus report covering all 25 constitutional invariant test cases. 25/25 passed (100.00% pass rate). |
