...
Concise rules for building Flight Booking System UI. These rules cover the most important patterns and constraints to keep the UI consistent without over-specifying every detail.
...
Four nav items: Dashboard, Search Flights, My Bookings, Profile.
...
- Confirmed: background `#ECFDF5`, text `#009966`
- Pending: background `#FFFBEB`, text `#B45309`
- Cancelled: background `#FEF2F2`, text `#DC2626`

### Match Score Badges (Flights)

- Strong Match (80–100): background `#ECFDF5`, text `#009966`
- Fair Match (60–79): background `#EFF6FF`, text `#2563EB`
- Weak Match (below 60): background `#FFF7ED`, text `#EA580C`

### Trend Badges (Dashboard Stats)

Use `border-radius: 4px` (not pill) with `#ECFDF5` background and `#009966` text.
...
**Danger button** (cancel booking, etc.):
...
Each flight result is a card showing:

- Airline logo + flight number (left)
- Departure → Arrival with times and duration (center)
- Match score badge (right of center)
- Price + action buttons (far right)
- Stops indicator: "Non-stop" / "1 Stop" / "2+ Stops"

Row border: `1px solid #E7EAF3` between results
