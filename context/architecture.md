...
2. **Mandatory Security Gateway (`apps/agent/src/agent/guardrails/gateway.py`)**:
   ...
   - Tool-result validation is a fixed fail-closed chain: size/iterative structure bounds (max 64 KiB, depth <= 5, nodes <= 500), strict minimized result-schema projection, PII scanning, then untrusted-content injection detection. No invalid result becomes a `ValidatedToolResult`.
   - The six tool-facing NestJS client operations stream decompressed response bytes through a 64 KiB bound before JSON loading; missing or false `Content-Length` values cannot bypass the cumulative check, and pre-parse JSON delimiter scanning rejects structures with depth > 5 or nodes > 500.
   - Upstream NestJS agent-gateway search and booking readiness endpoints provide shallow projections (depth <= 5), with the agent runner reconstituting public `sections[].fields[]` structures for the SSE `ACTION_REQUIRED` event to ensure web UI compatibility.
