...
- **Browser Transport & Correlation**: Chat clients stream directly to the public FastAPI agent endpoint (bypassing the Next.js proxy during the direct transport phase). Independently sanitized opaque trace and correlation IDs propagate across browser, agent, and backend; agent and API telemetry use closed allowlists and fixed event names, and audit metadata never stores request/session/user/offer/message/token/passenger/payment/passport values.
