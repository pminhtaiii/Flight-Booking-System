...
- **Authoritative Payment Validation and Binding (Phase 5)**: Payment creation validates before idempotency acquisition, attempt mutation, `AWAITING_PAYMENT`, customer creation, or Stripe. An expiring token/expiry pair provides the transient lease without adding a durable selection state; active leases block edits, expired leases do not. Duffel repricing occurs once outside database transactions, authoritative decimal totals are converted to minor units with exact digit parsing, and Payment plus the `PAYMENT_BOUND` transition persist atomically with PII-safe selection metadata. Base-fare payments remain compatible when no ancillary selection identity is supplied.
- **Immutable Order Recovery (Phase 5)**: Supplier-order creation and every existing confirmation/recovery checkpoint load seat and baggage rows through `Payment.ancillarySelection`, never through `BookingIntent.currentAncillarySelection`. Canonical order services count each seat row once, sum persisted baggage quantities by service ID, and retain the existing stable order/capture idempotency keys and authorization/order/capture/cancellation/compensation path. This preserves version N reconstruction if version N+1 later becomes current.
- **Phase 5 Verification Status (2026-07-29)**: Prisma schema validation, API typecheck, direct Nest build, and scoped lint of Phase 5-changed files passed. Database-backed migration/E2E verification is incomplete: the reachable local database diverges after `20260727000000_add_ancillary_checkout`, has unapplied local migration `20260729000000_add_ancillary_validation_lease`, and contains non-local migration `20260727044054_anything`. Two bounded direct Jest selections found zero tests due path-selection mismatch and were stopped without changing tests or code. Phase 5 test/migration gates T051 and T052 therefore remain open.
