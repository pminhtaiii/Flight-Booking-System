...
### Planned Ancillary Services Flow (Feature 15; Deterministic Path — No AI)
...
- **Data Schema**: Append-only, versioned AncillarySelection snapshots keyed by stable service/passenger/segment identities. BookingIntent points to the current version; Payment references the immutable version it priced for recovery.
...
- **Checkout Foundation (Upcoming / Planned)**: A planned baseline structure designed to protect checkout steps via a `protectCheckoutRoute` server helper and a `NEXT_PUBLIC_FEATURE_FLAG_CHECKOUT` flag. When implemented, it will resolve owner/active validations for `[intentId]` endpoints (rendering 404/403/410/500 layouts), collect passenger lists dynamically (applying traveler profile prefill, date formats, and international passport validation), and support e2e mock pipelines via custom cookies. (Note: These files are not yet committed or active in the repository branch).
