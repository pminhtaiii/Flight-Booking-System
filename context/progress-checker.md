...
- Logout sends a backend token-revocation request only to an explicitly configured public API origin; it never sends bearer credentials to a localhost fallback when the origin is absent.
