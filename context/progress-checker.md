...
- **Phase 6 Review Issues Remediation (2026-09-13)**:
  - **Issue 1 (Mock server port collision & fail-closed setup)**: In `apps/web/tests/security-boundaries.spec.ts`, `beforeAll` probes `http://127.0.0.1:3001/health`. If real NestJS API is running on port 3001, provisions test user `security-audit@example.test` with password `AuditPassword123!` via `POST /api/auth/register` (handles 201/200/409) and verifies credentials; if real API is absent, starts mock server on 3001 and rejects immediately on binding error (`server.once('error', reject)`).
