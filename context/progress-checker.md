...
- [x] Phase 2 / Slice 2: Cross-Service Health Contracts (T009–T014) (2026-08-27):
  ...
  - **T013: API-to-Agent Health Client Implementation (`apps/api/src/health/agent-health.service.ts`)**:
    - Updated `checkAgentHealth` target to `${agentUrl.replace(/\/+$/, '')}/health/live` with bounded 2000ms `AbortController` timeout, safe JSON narrowing from unknown without unsafe casts, sanitized `{ status: 'down' }` return on failure, and sanitized logger with `[checkAgentHealth]` context prefix omitting raw error messages. Synchronized `apps/api/test/agent-health.unit.ts`.
    - GREEN verification confirmed: 9/9 tests passed in `agent-health.service.spec.ts`, 6/6 passed in `agent-health.unit.ts`.
