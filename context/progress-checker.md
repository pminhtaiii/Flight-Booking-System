...
- Resolved `security-sast` CI failure: removed unsupported `- tsx` from Semgrep rule definitions in `tests/security/sast/ruleset.yml` and `tests/security/sast/guardrails.yml`; updated `tests/security/sast-runner.test.mjs` expectedLanguages (38/38 tests passing). Fixed Semgrep exit status 7 by unversioning standard registry rulesets (`p/default`, `p/owasp-top-ten`, `p/security-audit`, `p/secrets`) in `scripts/security/run-sast.mjs` and stripping `@.*` before passing to CLI, preventing 404 download crashes.
...
- Verified full static contract and security suites: `ci-workflow.contract.test.mjs` (23/23 passing), `evaluate-results.test.mjs` (38/38 passing), `sast-runner.test.mjs` (38/38 passing), `supply-chain.test.mjs` (17/17 passing), `@shared/types` (110/110 passing), API/Web typechecks (0 errors), and ESLint (0 errors, 0 warnings).
