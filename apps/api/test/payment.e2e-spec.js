...
describe('Payment (E2E)', () => {
    ...
    async function createBookingIntent(userId, offerId, overrides = {}) {
        const now = new Date();
    ...
    describe('POST /api/bookings/payment/create', () => {
        it('creates payment with valid booking intent (201)', async () => {
            ...
            expect(stripeService.createCustomer).toHaveBeenCalledWith('usera@example.com', undefined, expect.stringContaining('customer-create:'));
            expect(stripeService.createPaymentIntent).toHaveBeenCalledWith(12550, // amount in cents
            'USD', 'cus_mock_123', { bookingIntentId: intent.id }, expect.stringContaining('-stripe-intent'), undefined, undefined);
        });
        ...
        it('reuses existing Stripe customer when user already has stripeCustomerId', async () => {
            ...
            expect(createPaymentIntentSpy).toHaveBeenCalledWith(expect.any(Number), expect.any(String), 'cus_existing_789', expect.any(Object), expect.any(String), undefined, undefined);
        });
    });
    ...
    describe('DELETE /api/bookings/payment/methods/:methodId', () => {
        ...
        it('deletes own payment method (204)', async () => {
            ...
            const detachSpy = jest.spyOn(stripeService, 'detachPaymentMethod').mockResolvedValue({});
            await (0, supertest_1.default)(app.getHttpServer())
                .delete(`/api/bookings/payment/methods/${method.id}`)
                .set('Authorization', `Bearer ${tokenA}`)
                .expect(204);
