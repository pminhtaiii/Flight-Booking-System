...
const lockout_service_1 = require("@/auth/rate-limit/lockout.service");
...
describe('Adversarial and Edge Case Tests (E2E)', () => {
    ...
    beforeAll(async () => {
        ...
        lockoutService = moduleFixture.get(lockout_service_1.LockoutService);
    });
    afterAll(async () => {
        await app.close();
    });
    beforeEach(async () => {
        ...
        // Clear lockouts
        await (0, supertest_1.default)(app.getHttpServer()).post('/auth/test/reset-lockout').send({ clearAll: true });
    });
    ...
    describe('CacheService Fallback Mechanism (Redis Offline simulation)', () => {
        ...
        beforeEach(() => {
            originalRedisClient = cacheService.redisClient;
        });
        afterEach(() => {
            cacheService.redisClient = originalRedisClient;
        });
        it('should fall back to in-memory store and allow LockoutService to function when Redis throws errors', async () => {
            ...
            cacheService.redisClient = failingRedis;
    ...
    describe('Audit Log Sanitization and Robustness Boundaries', () => {
        ...
        it('should throw circular reference error on stringify when metadata is circular, or fail safely if handled', async () => {
            const circularMetadata = { name: 'circular' };
            ...
            expect(log).toBeDefined();
            expect((log?.metadata).self).toBe('[Circular]');
        });
    });
