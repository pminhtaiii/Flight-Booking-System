...
const duffel_event_processor_1 = require("@/disruption/webhook/duffel-event.processor");
...
describe('Disruption & Flight-Change Management (Webhook & Processor E2E)', () => {
    ...
    beforeAll(async () => {
        ...
        processor = moduleFixture.get(duffel_event_processor_1.DuffelEventProcessor);
    ...
    const generateSignatureHeader = (timestamp, rawBody, secret) => {
    ...
    describe('DuffelEventProcessor (Async Processor)', () => {
        it('should process pending inbox event, invoke sync, and complete successfully', async () => {
            ...
            expect(booking?.disruptionStatus).toBe(client_1.DisruptionStatus.DETECTED);
        });
    ...
    describe('Disruption APIs (Phase 6)', () => {
        ...
        it('populates extended fields (currentItinerary and disruption) in list and details endpoints', async () => {
            ...
            const rev = await prisma.itineraryRevision.create({
                data: {
                    ...
                    incrementalDiff: { presentationSummary: { details: 'incremental' } },
                    cumulativeDiff: { presentationSummary: { details: 'cumulative' } },
                    isMaterial: true,
                    segments: {
                        create: [
                            {
            ...
            const detailsRes = await (0, supertest_1.default)(app.getHttpServer())
            ...
            const listRes = await (0, supertest_1.default)(app.getHttpServer())
                ...
                .set('Authorization', `Bearer ${userToken}`)
                .expect(200);
            const listedBooking = listRes.body.bookings.find((b) => b.id === bookingId);
            ...
            const detailsResDisabled = await (0, supertest_1.default)(app.getHttpServer())
                .get(`/api/bookings/${bookingId}`)
                .set('Authorization', `Bearer ${userToken}`)
                .expect(200);
        ...
        it('returns disruption history with pagination and order', async () => {
            ...
            await prisma.itineraryRevision.create({
                data: {
                    ...
                    version: 1,
                    fingerprint: 'test-fingerprint-1',
                    incrementalDiff: { presentationSummary: { details: 'inc-1' } },
                    cumulativeDiff: { presentationSummary: { details: 'cum-1' } },
                    isMaterial: true,
            ...
            await prisma.itineraryRevision.create({
                data: {
                    ...
                    incrementalDiff: { presentationSummary: { details: 'inc-2' } },
                    cumulativeDiff: { presentationSummary: { details: 'cum-2' } },
                    isMaterial: true,
        ...
        it('transitions state atomically and idempotently for lifecycle actions (acknowledge, accept)', async () => {
            ...
            // 1. Acknowledge
            const ackRes1 = await (0, supertest_1.default)(app.getHttpServer())
            ...
            const ackRes2 = await (0, supertest_1.default)(app.getHttpServer())
                .post(`/api/bookings/${bookingId}/disruptions/${mockRevision.id}/acknowledge`)
                .set('Authorization', `Bearer ${userToken}`)
                .expect(200);
            expect(ackRes2.body.disruptionStatus).toBe('ACKNOWLEDGED');
            // 2. Accept
            const acceptRes1 = await (0, supertest_1.default)(app.getHttpServer())
            ...
            const acceptRes2 = await (0, supertest_1.default)(app.getHttpServer())
                .post(`/api/bookings/${bookingId}/disruptions/${mockRevision.id}/accept`)
                .set('Authorization', `Bearer ${userToken}`)
                .expect(200);
            ...
            const staleAck = await (0, supertest_1.default)(app.getHttpServer())
                .post(`/api/bookings/${bookingId}/disruptions/${mockRevision.id}/acknowledge`)
                .set('Authorization', `Bearer ${userToken}`)
                .expect(409);
