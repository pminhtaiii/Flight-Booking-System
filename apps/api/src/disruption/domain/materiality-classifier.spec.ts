...
describe('MaterialityClassifier', () => {
  ...
  describe('Threshold and Boundary Rules', () => {
    it('should evaluate DEPARTURE_MOVED_EARLIER strictly past 60 minutes', () => {
      ...
      const diff59 = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: -59 } as unknown as SegmentDiff] };
      expect(classifyMateriality(diff59, diff59).isMaterial).toBe(false);

      // 60m earlier: non-material (exact threshold)
      const diff60 = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: -60 } as unknown as SegmentDiff] };
      expect(classifyMateriality(diff60, diff60).isMaterial).toBe(false);

      // 61m earlier: material
      const diff61 = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: -61 } as unknown as SegmentDiff] };
    ...
    it('should evaluate DEPARTURE_MOVED_LATER strictly past 120 minutes', () => {
      ...
      const diff119 = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: 119 } as unknown as SegmentDiff] };
      expect(classifyMateriality(diff119, diff119).isMaterial).toBe(false);

      // 120m later: non-material (exact threshold)
      const diff120 = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: 120 } as unknown as SegmentDiff] };
      expect(classifyMateriality(diff120, diff120).isMaterial).toBe(false);

      // 121m later: material
      const diff121 = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: 121 } as unknown as SegmentDiff] };
    ...
    it('should evaluate FINAL_ARRIVAL_MOVED_EARLIER strictly past 60 minutes', () => {
      const diff60 = { ...emptyDiff, sliceDiffs: [{ finalArrivalShiftMinutes: -60 } as unknown as SliceDiff] };
      expect(classifyMateriality(diff60, diff60).isMaterial).toBe(false);

      const diff61 = { ...emptyDiff, sliceDiffs: [{ finalArrivalShiftMinutes: -61 } as unknown as SliceDiff] };
    ...
    it('should evaluate FINAL_ARRIVAL_MOVED_LATER strictly past 120 minutes', () => {
      const diff120 = { ...emptyDiff, sliceDiffs: [{ finalArrivalShiftMinutes: 120 } as unknown as SliceDiff] };
      expect(classifyMateriality(diff120, diff120).isMaterial).toBe(false);

      const diff121 = { ...emptyDiff, sliceDiffs: [{ finalArrivalShiftMinutes: 121 } as unknown as SliceDiff] };
  ...
  describe('Incremental vs Cumulative Baselines', () => {
    it('should correctly flag when only cumulative drift is material', () => {
      ...
      const incremental = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: 70 } as unknown as SegmentDiff] };
      // Cumulative change is major (e.g. two minor changes totalling +140m later, 140 > 120)
      const cumulative = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: 140 } as unknown as SegmentDiff] };
    ...
    it('should correctly flag when only incremental is material (unlikely but possible or theoretically separate)', () => {
      const incremental = { ...emptyDiff, segmentDiffs: [{ departureTimeShiftMinutes: 130 } as unknown as SegmentDiff] };
