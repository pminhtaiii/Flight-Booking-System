...
describe('PaymentStateMachine', () => {
  ...
  describe('enforceTransition', () => {
    ...
    it('throws BadRequestException on invalid transition', () => {
      expect(() => enforceTransition(PaymentStatus.CREATED, PaymentStatus.SUCCEEDED)).toThrow(BadRequestException);
    });
  });
  ...
  describe('getPreDisputeStatus', () => {
    it('returns valid status when current status is SUCCEEDED, PARTIALLY_REFUNDED, or REFUNDED', () => {
      ...
      expect(getPreDisputeStatus(PaymentStatus.PARTIALLY_REFUNDED)).toBe(PaymentStatus.PARTIALLY_REFUNDED);
  ...
  describe('resolveDisputeStatus', () => {
    ...
    it('resolves lost disputes to CHARGEBACK_LOST', () => {
      ...
      expect(resolveDisputeStatus('lost', PaymentStatus.PARTIALLY_REFUNDED)).toBe(PaymentStatus.CHARGEBACK_LOST);
    ...
    it('throws BadRequestException on invalid dispute outcome', () => {
      expect(() => resolveDisputeStatus('invalid' as unknown as 'won', PaymentStatus.SUCCEEDED)).toThrow(BadRequestException);
    });
  });
});
