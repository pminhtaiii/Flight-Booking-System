...
export class BookingService {
  ...
  async listBookings(userId: string, tab: BookingTab, page: number, limit: number): Promise<BookingListResponseDto> {
    if (!this.bookingManagementService) {
      throw new Error('BookingManagementService is not available');
    }
    return this.bookingManagementService.listBookings(userId, tab, page, limit);
  }
  ...
  async getBookingDetail(bookingId: string, userId: string): Promise<BookingDetailResponseDto> {
    ...
    return this.bookingManagementService.getBookingDetail(bookingId, userId);
  }
