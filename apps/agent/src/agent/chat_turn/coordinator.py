...
logger = logging.getLogger("agent.chat_turn.coordinator")
...
async def _persist_response(
    ...
    coro = client.create_message_batch(session_id, payload)
    if use_shield:
...
class TurnSessionCoordinator:
    ...
    def _get_redis_client(self) -> object:
    ...
    async def _finalize_cleanup(
        ...
        *,
        ...
        if (
            not persisted
            and (bool(partial_response and partial_response.strip()) or force_persist)
            ...
            try:
                ...
                if fence_valid:
                    ...
                    try:
                        awaitable = asyncio.shield(persist_task) if use_shield else persist_task
                        batch_res = await asyncio.wait_for(awaitable, timeout=3.0)
                    except (TimeoutError, asyncio.CancelledError):
                        # Never close the pipeline or release the lease while a batch
                        # created by this cleanup can still be written.
                        persist_task.cancel()
        ...
        try:
            ...
            if redis_client is not None:
                try:
                    ...
                    lifecycle = self._snapshot_lifecycle_type(repo)
            ...
            if self.gateway is not None:
                ...
                pipeline = self._output_stream_session_type(
                    context=mem_context,
                    config=output_config,
                    session_id=session_id,
                )
            ...
            snapshot_lifecycle = (
                self._snapshot_lifecycle_type(TrustedSnapshotRepository(redis_client))
                if redis_client is not None
                else None
            )
            ...
            try:
                ...
                _, _, err_event = await self._finalize_cleanup(
                    ...
                    force_persist=force_persistence or (exc.error_code == "HANDOFF_FAILED"),
                )
                pipeline = None
                req_id = None
                released = True
