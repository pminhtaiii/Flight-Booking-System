...
def test_stream_message_too_long(monkeypatch):
    ...
    response = client.post(
        "/chat/stream", json={"message": "a" * 11, "sessionId": None}, headers=headers
    )
    assert response.status_code == 400
    assert (
        response.json()["detail"] == "Message exceeds maximum length"
        or response.json().get("code") == "MESSAGE_TOO_LONG"
    )
...
def test_stream_guardrails_unavailable(monkeypatch):
    ...
    response = client.post(
        "/chat/stream", json={"message": "hello", "sessionId": None}, headers=headers
    )
...
def test_stream_guardrails_blocked(monkeypatch):
    ...
    mock_guardrail.validate_message = AsyncMock(
        return_value=(False, "safety violation: input blocked")
    )
    monkeypatch.setattr(app.state, "guardrails", mock_guardrail, raising=False)

    with client.stream(
        "POST", "/chat/stream", json={"message": "hello", "sessionId": None}, headers=headers
    ) as response:
...
def test_stream_with_valid_session_id(monkeypatch):
    ...
    async def mock_astream_events(*args, **kwargs):
        yield {"event": "on_chat_model_stream", "data": {"chunk": AIMessageChunk(content="This ")}}
        yield {"event": "on_chat_model_stream", "data": {"chunk": AIMessageChunk(content="is ")}}
        yield {"event": "on_chat_model_stream", "data": {"chunk": AIMessageChunk(content="a ")}}
        yield {"event": "on_chat_model_stream", "data": {"chunk": AIMessageChunk(content="mock ")}}
        yield {
...
async def test_stream_omitted_session_id(monkeypatch):
    ...
    # Mock NestJSClient
    with patch(
        "agent.tools.nestjs_client.NestJSClient.create_session", new_callable=AsyncMock
