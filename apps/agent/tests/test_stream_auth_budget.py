...
def test_jwt_missing_required_claims_rejected():
    ...
    res = client.post("/chat/stream", json={"message": "hello"}, headers={"Authorization": f"Bearer {token}"})
    assert res.status_code == 401
    assert "Invalid token" in res.json().get("detail", "") or "claims" in res.json().get("detail", "").lower()
...
def test_nestjs_access_check_revoked_or_deactivated_rejected():
    ...
    with patch("agent.streaming.sse.NestJSClient") as MockClient:
        ...
        res = client.post("/chat/stream", json={"message": "hello"}, headers={"Authorization": f"Bearer {token}"})
        assert res.status_code == 401
        assert "unauthorized" in res.json().get("detail", "").lower() or "revoked" in res.json().get("detail", "").lower() or "inactive" in res.json().get("detail", "").lower()
...
def test_zero_quota_charged_and_zero_inference_on_auth_failure():
    ...
    with patch("agent.repositories.chat_budget_repository.ChatBudgetRepository.admit_request", new_callable=AsyncMock) as mock_quota, \
         patch("agent.streaming.sse.graph.astream_events") as mock_graph, \
         patch("agent.streaming.sse._persist_response") as mock_persist:
        
        res = client.post("/chat/stream", json={"message": "hello"}, headers={"Authorization": invalid_token})
        assert res.status_code == 401
...
def test_zero_quota_charged_and_zero_inference_on_access_check_failure():
    token = make_token()
    with patch("agent.streaming.sse.NestJSClient") as MockClient, \
         patch("agent.repositories.chat_budget_repository.ChatBudgetRepository.admit_request", new_callable=AsyncMock) as mock_quota, \
         patch("agent.streaming.sse.graph.astream_events") as mock_graph, \
         patch("agent.streaming.sse._persist_response") as mock_persist:
...
def test_cross_user_session_isolation_denied():
    ...
    with patch("agent.streaming.sse.NestJSClient") as MockClient, \
        ...
        mock_nestjs.get_memory.side_effect = Exception("CHAT_SESSION_NOT_FOUND: Session not owned by user")
        MockClient.return_value = mock_nestjs
