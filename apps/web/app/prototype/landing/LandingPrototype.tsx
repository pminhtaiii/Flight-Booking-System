...
> = {
  ...
  orbit: {
    ...
    description:
      'A cinematic, map-led booking experience that turns routes, preferences, and possibilities into one calm decision.',
    label: 'Orbit view',
  },
  itinerary: {
    ...
    description:
      'A warm, editorial approach to intelligent travel — the system learns what matters and quietly handles the complexity.',
  ...
  return (
    <main className={`${styles.page} ${styles[selectedVariant]}`}>
      <p className={styles.prototype}>
        Prototype — disposable visual exploration, not production UI
      </p>
      <nav className={styles.nav} aria-label="Prototype navigation">
        <Link className={styles.brand} href="/prototype/landing">
          wayfinder<span>°</span>
        </Link>
      ...
      <section className={styles.trust} aria-label="Product capabilities">
        <p>Personalized flight intelligence</p>
        <span>•</span>
        <p>Decisions explained clearly</p>
        <span>•</span>
        <p>Booking stays dependable</p>
      </section>
      ...
      <div className={styles.variantBar} aria-label="Prototype variants">
        ...
        {(['copilot', 'orbit', 'itinerary'] as LandingVariant[]).map((item: LandingVariant) => (
          <Link
            className={item === selectedVariant ? styles.selected : styles.variantLink}
            href={`/prototype/landing?variant=${item}`}
            key={item}
          >
...
function CopilotVisual(): JSX.Element {
  return (
    <div className={styles.copilotVisual}>
      <div className={styles.glow} />
      <div className={styles.assistantCard}>
        <p>
          <span>✦</span> Wayfinder knows you prefer
        </p>
        <h2>
          Quiet departures
          <br />
      ...
      <div className={styles.pulse}>Listening for what matters</div>
    </div>
  );
}
...
function OrbitVisual(): JSX.Element {
  return (
    <div className={styles.orbitVisual}>
      ...
      <div className={`${styles.city} ${styles.cityOne}`}>
        Tokyo <small>09:45</small>
      </div>
      <div className={`${styles.city} ${styles.cityTwo}`}>
        Ho Chi Minh <small>07:10</small>
      </div>
      <div className={styles.mapCard}>
        <p>Route clarity</p>
        <strong>One effortless choice</strong>
        <span>Non-stop · 5h 35m</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className={styles.itineraryVisual}>
      <div className={styles.day}>
        <span>08</span>
        <p>Leave room for wonder</p>
        <i>Departure day</i>
      </div>
      <div className={styles.ticket}>
        <p>
          HO CHI MINH CITY <span>→</span> TOKYO
        </p>
        <strong>07:10 — 14:45</strong>
        <small>Your ideal morning departure</small>
      </div>
      <div className={styles.day}>
        <span>12</span>
        <p>Take the scenic route home</p>
        <i>Return day</i>
      </div>
    </div>
  );
