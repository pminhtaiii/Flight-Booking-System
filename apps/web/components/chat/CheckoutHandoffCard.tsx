  ...
  return (
    <div className="card rounded border border-card-border bg-card p-4 space-y-3">
      ...
      <div className="flex flex-col gap-1 text-sm text-text-secondary">
        <div>
          <span className="font-medium text-text-primary">Airline:</span> {event.display.airline}
        </div>
        <div>
          <span className="font-medium text-text-primary">Route:</span> {event.display.origin} to{' '}
          {event.display.destination}
        </div>
        <div>
          <span className="font-medium text-text-primary">Departure:</span>{' '}
          {new Date(event.display.departureAt).toLocaleString()}
        </div>
        <div>
          <span className="font-medium text-text-primary">Arrival:</span>{' '}
          {new Date(event.display.arrivalAt).toLocaleString()}
        </div>
        <div>
          <span className="font-medium text-text-primary">Price:</span> {event.display.price}{' '}
          {event.display.currency}
        </div>
      </div>
      <form
        ref={formRef}
