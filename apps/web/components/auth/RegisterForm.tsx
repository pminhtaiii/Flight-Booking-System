      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1 text-xs mt-2 border border-secondary-border rounded-lg p-2.5 bg-card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1">
            <div className={`flex items-center gap-1.5 ${checks.length ? "text-text-confirmed" : "text-text-secondary"}`}>
              <span>{checks.length ? "✓" : "•"}</span>
              <span>8-128 characters</span>
            </div>
            <div className={`flex items-center gap-1.5 ${checks.uppercase ? "text-text-confirmed" : "text-text-secondary"}`}>
              <span>{checks.uppercase ? "✓" : "•"}</span>
              <span>One uppercase letter</span>
            </div>
            <div className={`flex items-center gap-1.5 ${checks.lowercase ? "text-text-confirmed" : "text-text-secondary"}`}>
              <span>{checks.lowercase ? "✓" : "•"}</span>
              <span>One lowercase letter</span>
            </div>
            <div className={`flex items-center gap-1.5 ${checks.digit ? "text-text-confirmed" : "text-text-secondary"}`}>
              <span>{checks.digit ? "✓" : "•"}</span>
              <span>One digit</span>
            </div>
            <div className={`flex items-center gap-1.5 ${checks.special ? "text-text-confirmed" : "text-text-secondary"}`}>
