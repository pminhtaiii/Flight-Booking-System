...
export const validateQuickSearch = (params: QuickSearchParams, today: Date = new Date()): QuickSearchValidation => {
  ...
  if (!IATA_CODE_PATTERN.test(origin)) return { valid: false, error: 'Departure airport code must be a three-letter IATA code.' };
  if (!IATA_CODE_PATTERN.test(destination)) return { valid: false, error: 'Arrival airport code must be a three-letter IATA code.' };
  if (origin === destination) return { valid: false, error: 'Departure and arrival airports must be different.' };
  if (!isCalendarDate(params.departureDate)) return { valid: false, error: 'Departure date must be a valid YYYY-MM-DD date.' };

  const [year, month, day] = params.departureDate.split('-').map(Number);
  const departureDate = new Date(year, month - 1, day);
  if (departureDate < startOfLocalDay(today)) return { valid: false, error: 'Departure date must be today or later.' };
