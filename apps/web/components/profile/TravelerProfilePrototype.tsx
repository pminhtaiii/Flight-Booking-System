...
function getSectionStatus(values: Record<string, string>): 'complete' | 'attention' {
  return Object.values(values).every((value: string) => value.trim().length > 0) ? 'complete' : 'attention';
}
    ...
    return (
      <label className={styles.field} htmlFor={`${section}-${field}`}>
        ...
        <select
          ...
          value={value}
        >
          {options.map((option: SelectOption) => <option key={option.value} value={option.value}>{option.label}</option>)}
        </select>
      </label>
    );
  }
      ...
      <div className={styles.sectionHeader}>
        <div>
          <div className={styles.sectionTitleRow}>
            ...
            <span className={`${styles.status} ${status === 'complete' ? styles.statusComplete : styles.statusAttention}`}>
      ...
      <div className={styles.shell}>
        <header className={styles.workspaceHeader}>
          ...
          <div className={styles.workspaceControls}>
            <span className={styles.secureBadge}><span className={styles.syncDot} aria-hidden="true" />Secure session</span>
            <span className={styles.avatarChip} aria-label="Signed in as Maya Chen">MC</span>
          </div>
        </header>

        <div className={styles.workspaceBar}>
          <nav className={styles.breadcrumb} aria-label="Workspace breadcrumb">
            <span>Workspace</span><span className={styles.breadcrumbSlash} aria-hidden="true">/</span><span>Traveler profile</span>
          </nav>
          <div className={styles.contextActions} aria-label="Prototype context">
            <span>Readiness workspace</span><span>Local preview</span>
          </div>
        </div>
        ...
        <section className={styles.hero}>
          ...
          <div className={styles.completionCard} aria-label={`${completion}% profile completion`}>
            ...
            <p>{completion === 100 ? 'Everything is ready.' : 'A few details need your attention before an international booking.'}</p>
          </div>
        </section>

        <section className={styles.signalStrip} aria-label="Profile health summary">
          <div className={styles.signalItem}><span className={styles.signalLabel}>Profile health</span><strong className={styles.signalValue}>{completion}% ready</strong><span className={styles.signalGood}>On track</span></div>
          <div className={styles.signalItem}><span className={styles.signalLabel}>Open detail</span><strong className={styles.signalValue}>{completion === 100 ? 'None' : 'Phone + passport'}</strong><span className={styles.signalAccent}>{completion === 100 ? 'Clear' : '2 items'}</span></div>
          <div className={styles.signalItem}><span className={styles.signalLabel}>Privacy posture</span><strong className={styles.signalValue}>Owner-scoped</strong><span className={styles.signalGood}>Protected</span></div>
          <div className={styles.signalItem}><span className={styles.signalLabel}>Revision</span><strong className={styles.signalValue}>01</strong><span className={styles.signalNeutral}>In memory</span></div>
        </section>

        <div className={styles.privacyNotice} role="note">
          <span className={styles.privacyIcon} aria-hidden="true">✓</span>
          <div><strong>Your information stays private.</strong><span>This prototype never sends or stores these mock values. Production profile responses will be no-store and owner-scoped.</span></div>
        </div>

        {saveState === 'saved' ? <div className={styles.successAlert} role="status">Saved in this prototype. The next save demonstrates how a stale revision is handled.</div> : null}
        {saveState === 'conflict' ? <div className={styles.conflictAlert} role="alert"><strong>This profile changed in another tab.</strong><span>Reload the latest profile before saving again so no traveler details are overwritten.</span><button className="btn-secondary" onClick={handleReset} type="button">Reload mock profile</button></div> : null}
        ...
        <form className={styles.form} onReset={handleReset} onSubmit={handleSave}>
              ...
              <div className={`${styles.field} ${styles.phoneField}`}>
                ...
                <div className={styles.phoneControl}>
                  <label className={styles.phoneCountry} htmlFor="contact-phoneCountryCode">
                    ...
                    >
                      {phoneCountryOptions.map((option: SelectOption) => <option key={option.value} value={option.value}>{option.label}</option>)}
                    </select>
                  </label>
            ...
            <div className={styles.fieldGrid}>
              ...
              <label className={styles.field} htmlFor="travelDocument-passportNumber">
                ...
                <div className={styles.passwordControl}>
                  ...
                  />
                  <button className={styles.showButton} onClick={() => setShowPassport((current: boolean) => !current)} type="button">{showPassport ? 'Hide' : 'Show'}</button>
                </div>
                <span className={styles.fieldHint}>Visible only while you are editing this secure field.</span>
              </label>
          ...
          <div className={styles.actions}>
            <div><strong>Revision 1</strong><span>Last updated just now · Prototype only</span></div>
            <div className={styles.actionButtons}><button className="btn-secondary" onClick={handleReset} type="reset">Cancel</button><button className="btn-primary" type="submit">Save profile</button></div>
          </div>
        </form>

        <p className={styles.footerNote}>Prototype question: does a single page make it easier to review and correct every traveler field?</p>
      </div>
    </main>
  );
}
