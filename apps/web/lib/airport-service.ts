...
export async function searchAirports(q: string, limit?: number): Promise<Airport[]> {
  try {
    ...
    const res = await fetch(`${API_URL}/api/airports/search?q=${encodeURIComponent(q)}${limitQuery}`);
    if (!res.ok) {
...
export async function getAirportByIataCode(iataCode: string): Promise<Airport | null> {
  try {
    const res = await fetch(`${API_URL}/api/airports/${encodeURIComponent(iataCode.toUpperCase())}`);
    if (!res.ok) {
...
export async function getNearbyAirports(
  ...
  try {
    ...
    const res = await fetch(`${API_URL}/api/airports/nearby?${params.toString()}`);
    if (!res.ok) {
